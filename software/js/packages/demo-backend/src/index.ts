...
const start = async () => {
  ...
  if (matchingPorts.length < 1) {
    console.error(
      `No smartknob usb serial port found! ${JSON.stringify(
        ports,
        undefined,
        4
      )}`
    ...
    console.error(
      `Multiple smartknob usb serial ports found: ${JSON.stringify(
        matchingPorts,
        undefined,
        4
      )}`
  ...
  const smartknob = new SmartKnob(
    ...
    (message: PB.FromSmartKnob) => {
      if (message.payload === "log" && message.log) {
        ...
        const state = PB.SmartKnobState.toObject(
          message.smartknobState as PB.SmartKnobState,
          { defaults: true }
        );
        io.emit("state", { pb: message.smartknobState });
        if (
          message.smartknobState.currentPosition !==
            lastLoggedState?.currentPosition ||
          Math.abs(
            (message.smartknobState.subPositionUnit ?? 0) -
              (lastLoggedState?.subPositionUnit ?? 0)
          ) > 1
        ) {
